...
//this is very slightly better than it was because you can use it more places. still can't do \his[src] though.
	...
	if(perp_name && (HAS_TRAIT(user, TRAIT_SECURITY_HUD) || HAS_TRAIT(user, TRAIT_MEDICAL_HUD) || HAS_TRAIT(user, TRAIT_MEDICAL_HUD_MENTAL)))
		var/datum/data/record/record = find_record("name", perp_name, GLOB.data_core.general)
		if(record)
			. += "<span class='deptradio'>Rank:</span> [record.fields["rank"]]\n<a href='?src=[REF(src)];hud=1;photo_front=1'>\[Front photo\]</a><a href='?src=[REF(src)];hud=1;photo_side=1'>\[Side photo\]</a>"
		if(HAS_TRAIT(user, TRAIT_MEDICAL_HUD))
			...
			if(record)
				var/health_r = record.fields["p_stat"]
				. += "<a href='?src=[REF(src)];hud=m;p_stat=1'>\[[health_r]\]</a>"
				health_r = record.fields["m_stat"]
		...
		if(HAS_TRAIT(user, TRAIT_SECURITY_HUD))
			...
			//|| !user.canmove || user.restrained()) Fluff: Sechuds have eye-tracking technology and sets 'arrest' to people that the wearer looks and blinks at.
				...
				record = find_record("name", perp_name, GLOB.data_core.security)
				if(record)
					criminal = record.fields["criminal"]

...
///////HUDs///////
	if(href_list["hud"])
		...
		if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD) && !HAS_TRAIT(H, TRAIT_MEDICAL_HUD) && !HAS_TRAIT(H, TRAIT_MEDICAL_HUD_MENTAL))
			return
		var/datum/data/record/record = find_record("name", perpname, GLOB.data_core.general)
		if(href_list["photo_front"] || href_list["photo_side"])
			...
			if(href_list["photo_front"])
				P = record.fields["photo_front"]
			else if(href_list["photo_side"])
				P = record.fields["photo_side"]
			if(P)
				P.show(H)
			return
		...
		if(href_list["hud"] == "m")
			...
			if(!(ACCESS_MEDICAL in access))
				...
				return
			if(href_list["p_stat"])
				var/health_status = input(usr, "Specify a new physical status for this person.", "Medical HUD", record.fields["p_stat"]) in list("Active", "Physically Unfit", "*Unconscious*", "*Deceased*", "Cancel")
				if(!record)
					return
				if(!H.canUseHUD())
					return
				...
				if(health_status && health_status != "Cancel")
					record.fields["p_stat"] = health_status
				return
			if(href_list["m_stat"])
				var/health_status = input(usr, "Specify a new mental status for this person.", "Medical HUD", record.fields["m_stat"]) in list("Stable", "*Watch*", "*Unstable*", "*Insane*", "Cancel")
				if(!record)
					return
				if(!H.canUseHUD())
					return
				if(!HAS_TRAIT(H, TRAIT_MEDICAL_HUD))
					return
				if(health_status && health_status != "Cancel")
					record.fields["m_stat"] = health_status
		...
		if(href_list["hud"] == "s")
			...
			if(!record)
				...
				return
			if(href_list["status"])
				var/setcriminal = input(usr, "Specify a new criminal status for this person.", "Security HUD", record.fields["criminal"]) in list("None", "*Arrest*", "Incarcerated", "Paroled", "Discharged", "Cancel")
				if(setcriminal != "Cancel")
					...
					if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD))
						return
					investigate_log("[key_name(src)] has been set from [record.fields["criminal"]] to [setcriminal] by [key_name(usr)].", INVESTIGATE_RECORDS)
					record.fields["criminal"] = setcriminal
					sec_hud_set_security_status()
				return
			...
			if(href_list["view"])
				...
				if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD))
					return
				to_chat(usr, "<b>Name:</b> [record.fields["name"]] <b>Criminal Status:</b> [record.fields["criminal"]]")
				for(var/datum/data/crime/c in record.fields["crim"])
					...
					to_chat(usr, "----------")
				to_chat(usr, "<b>Notes:</b> [record.fields["notes"]]")
				return
			...
			if(href_list["add_citation"])
				...
				if(!record || !t1 || !fine || !allowed_access)
					return
				if(!H.canUseHUD())
					return
				if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD))
				...
				GLOB.data_core.addCitation(record.fields["id"], crime)
				investigate_log("New Citation: <strong>[t1]</strong> Fine: [fine] | Added to [record.fields["name"]] by [key_name(usr)]", INVESTIGATE_RECORDS)
				return

			if(href_list["add_crime"])
				var/t1 = stripped_input("Please input crime name:", "Security HUD", "", null)
				if(!record || !t1 || !allowed_access)
					return
				if(!H.canUseHUD())
					return
				if(!HAS_TRAIT(H, TRAIT_SECURITY_HUD))
				...
				GLOB.data_core.addCrime(record.fields["id"], crime)
				investigate_log("New Crime: <strong>[t1]</strong> | Added to [record.fields["name"]] by [key_name(usr)]", INVESTIGATE_RECORDS)
			...
			if(href_list["add_details"])
				...
				if(href_list["cdataid"])
					GLOB.data_core.addCrimeDetails(record.fields["id"], href_list["cdataid"], t1)
					investigate_log("New Crime details: [t1] | Added to [record.fields["name"]] by [key_name(usr)]", INVESTIGATE_RECORDS)
			...
			if(href_list["view_comment"])
				...
				var/counter = 1
				while(record.fields[text("com_[]", counter)])
					to_chat(usr, record.fields[text("com_[]", counter)])
					to_chat(usr, "----------")
					counter++
				return
			...
			if(href_list["add_comment"])
				...
				while(record.fields[text("com_[]", counter)])
					counter++
				record.fields[text("com_[]", counter)] = text("Made by [] on [] [], []<BR>[]", allowed_access, station_time_timestamp(), time2text(world.realtime, "MMM DD"), GLOB.year_integer+540, t1)
...
/mob/living/carbon/human/replace_records_name(oldname,newname) // Only humans have records right now, move this up if changed.
	for(var/list/L in list(GLOB.data_core.general,GLOB.data_core.medical,GLOB.data_core.security,GLOB.data_core.locked))
		var/datum/data/record/record = find_record("name", oldname, L)
		if(record)
